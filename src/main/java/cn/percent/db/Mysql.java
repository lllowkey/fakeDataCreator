...
public class Mysql {
    public void Mysql(String rdmsType) {
        /*读取json文件*/
...
//            String rdmsType = db.getRdmsConfig().getRdmsType();//获取数据库类型
            ...
            int calNumber = (int) Math.ceil(dbNumber * dbRatio);//需要建表的库的数量
...
//          UIDCreator uidCreator = new UIDCreator();
            ...
            HashSet<String> dbName = new HashSet<>();//存放需要建表的库/schema的名称
            for (int i = 0; i < dbNumber; i++) {
                uid = UUID.randomUUID().toString().replace("-", "").toLowerCase();//java自带方法生成uuid
                String uid1 = md5.md5(uid);
                String createDbName = rdmsType+"_DB_"+uid1+"_"+tm.timer();
                if( dbName.size() != calNumber) {
                    dbName.add(createDbName);
                }
                String createDB = "CREATE DATABASE "+createDbName
                        +" DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci";
//                System.out.println(createDB);
                dbStmt.executeUpdate(createDB);
            }
            /*while(it.hasNext()){
            ...
            Iterator<String> it = dbName.iterator();//迭代器
...
//                    System.out.println(randColumnStr);
                    String tbsql = "CREATE TABLE "+tableName+" ("+randColumnStr+")";
